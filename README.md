...
**Docker** image with **uWSGI** and **Nginx** for **Python 3.6** applications (as **Flask**) in a single container.
...
* You shouldn't have to clone the GitHub repo. You should use it as a base image for other images, using `FROM mxkd/uwsgi-nginx` in your `Dockerfile`.
